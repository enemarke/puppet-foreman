  ...
  on_os_under_test.each do |os, facts|
    context "on #{os}" do
      let :params do {
          ...
          :configure_passenger_repo => configure_passenger_repo,
      } end
      let(:serveraliases) { ['foreman', 'also.foreman'] }
      let(:priority){ '15' }
      let(:ssl_crl){ '/crl.pem' }
      let(:keepalive){ true }
      ...
      let(:configure_passenger_repo){ true }
      let(:ssl){ true }
      let(:keepalive_timeout){ 5 }

      let :facts do facts end

      describe 'with minimal parameters' do
        let(:serveraliases){ ['foreman'] }
        let(:ssl){ false }
      ...
      describe 'with keepalive parameters set' do
        let(:priority){ '05' }
        let(:keepalive){ false }
        let(:max_keepalive_requests){ 10 }
        let(:keepalive_timeout){ 15 }
      ...
      describe 'with configure_passenger_repo set to false' do
        let(:configure_passenger_repo){ false }
        it 'should not contain the repo file' do
          should_not contain_file('/etc/yum.repos.d/passenger.repo')
        end
      end
